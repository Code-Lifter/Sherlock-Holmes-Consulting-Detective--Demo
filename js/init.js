...
async function initializeGame() {
    ...
    const hasSave = loadFromLocalStorage(); // Attempt to load saved state
    
    updateLettersDisplay(); 
    updateLeadsCountDisplay(); 
    populateDropdown(); // Re-populate dropdown so visited/locked classes apply to loaded state

    if (hasSave && gameState.currentLocation) {
        visitLocation(gameState.currentLocation); // Resume where they left off
    } else {
        showIntroduction(); // Start fresh
    }
}
...
async function loadGameData() {
    try {
        const [locationsResponse, caseIntroResponse] = await Promise.all([
            ...
            fetch("./json/caseIntro.json") // Load intro data here as well
        ]);
...
async function showQuestions() {
    closeToolbox(); // <<< ADDED
    try {
        const response = await fetch('./json/questions.json'); // Fetch the questions data
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status} (${response.statusText})`);
        }
